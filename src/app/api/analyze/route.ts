...
### 2. tags（2-3个标签，精炼有质感，避免堆砌通用词）
请组合搭配，尽量从不同维度各取其一，优先选择最能体现本片独特气质的词：
- 荣誉（最高优先级）：奥斯卡获奖、奥斯卡提名、安妮奖、国际获奖
- 出品/工艺（可选）：皮克斯、迪士尼、吉卜力、国产佳作、独立短片、定格动画、手绘动画
- 主题/情感（必选1个）：成长、友情、亲情、勇气、梦想、治愈、温暖、励志、想象力
- 气质/看点（必选1个）：温馨、唯美、奇幻、幽默、童真、脑洞、热血、感动

要求：
- 【重要】结合你自己的知识判断荣誉：只要你确知本片获得过奥斯卡、安妮奖等重要奖项，就必须把荣誉作为第一个标签，绝不能因为简介里没提就遗漏（这是家长最看重的信息）
- 不确定是否获奖时，宁可不标，也不要编造奖项
- 不要每次都用"国外动画短片""动画短片"这类宽泛标签，除非确实没有更贴切的词
- 不要用"无对白""适合全家""老少皆宜"等观看门槛类描述作为标签（这类信息放在推荐语里说）
- 标签要短（2-5字），像影片的"气质标签"，让人一眼想点开
...
async function callGeminiAPI(prompt: string): Promise<string> {
  ...
  const response = await fetch(apiUrl, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.7,
      // gemini-2.5 系列为思考型模型，思考过程与输出共用 token 预算，
      // JSON 完整输出 + 思考需留足空间，否则 JSON 会被截断导致解析失败
      max_tokens: 4096,
    })
  })
